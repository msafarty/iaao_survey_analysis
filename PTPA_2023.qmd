...
ggplot2::ggsave("q10.png", plot = q10vis)
```
...
ggsave(plot=q13vis, filename='q13.png')
```
...
ggsave(plot=q15vis, filename=  'q15.png')
```
...
q21vis <- ggplot2::ggplot(q21, aes(y = sub_question, x = proportion, fill = intlGrp)) +
  geom_bar(stat = "identity", position = "dodge")+
  theme_minimal() + labs(title = stringr::str_wrap('Indicate which of the following types of reference materials your agency provides to assessors and other users',width=80), y = '') 

q21vis

ggsave(plot=q21vis,filename='q21.png')
```
...
ggsave(plot=q22vis,filename='q22.png')
```
...
ggsave(plot=q23vis, filename='q23.png')
```
...
ggplot2::ggsave(filename='q26.png',plot=q26vis)
...
ggplot2::ggsave(filename = 'q31.png', plot = q31vis)
```
...
ggplot2::ggsave(filename='q34.png',plot=q34vis)
```
...
ggplot2::ggsave(plot = q36vis, filename = 'q36.png')
...
q38vis <- ggplot(data = q38, aes(y = '', x = proportion, fill = intlGrp)) + 
  geom_bar(stat = "identity",  width = 0.4,
           position=position_dodge(width = 0.5))+
  theme_bw() + labs(title = q38title, y='Yes') +  geom_text(aes(label = n.x, fontface = 'bold'), size = 8, position = position_dodge(width = 0.5))

q38vis

ggplot2::ggsave(plot=q38vis, filename='q38vis.png') 
