...
q13vis_a <- ggplot2::ggplot(q13_a, aes(y = sub_question, x = proportion, fill = intlGrp)) +
  geom_bar(stat = "identity", position = "dodge")+
  theme_bw() + labs(title = stringr::str_wrap(title,width=65), subtitle = 'Property Set 1 - Assessed as Real, Personal, or Operating', x = 'Percent of Responses per Group', y = 'Property Type')  +  geom_text(aes(label = n.x, fontface = 'bold'), size = 4,position = position_dodge(width = 0.9), vjust = 0.5, hjust = 1) +   scale_x_continuous(labels = scales::percent_format()) +  labs(fill='Group with Counts') + scale_fill_manual(values = c("CANADA" = '#E69F00', "USA" = '#56B4E9', "INTL" = '#CC79A7')) + scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 25)) 
...
q22vis <- ggplot2::ggplot(q22, aes(x = response_merged, y = proportion, fill = intlGrp)) +
 ...
 labs(title = title, y = 'Percent of Responses per Group', x = 'Response')  +  geom_text(aes(label = n.x, fontface = 'bold'), size = 5,position = position_dodge(width = 0.9), vjust = 1.5) + labs(fill='Group with Counts') + scale_fill_manual(values = c("CANADA" = '#E69F00', "USA" = '#56B4E9', "INTL" = '#CC79A7')) + scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 25)) + theme(axis.text.x = element_text(size = 11)) + scale_y_continuous(scales::percent_format())
...
### Question 68
```{r echo=F}
# Table 12 from JPTAA PTAPP 2017- 
# give the percentage subquestion as a normal table JPTAA style so alan can edit
unique(q68$response_merged)
...
q40_multiYear <- tableData %>% dplyr::filter(question_num == 40 & sub_question_id != 3 & !is.na(X2017_response) & X2017_response != 'NULL' & X2017_response != 'NA') %>% dplyr::mutate(tableResponse_sq1_2017 = dplyr::case_when(
...
q40_multiYear_noINTL <- q40_multiYear %>% #dplyr::filter(intlGrp != 'INTL' & sub_question_id == 1) 
dplyr::group_by(category_desc, intlGrp) %>% dplyr::summarise(
  ...
  ) %>% data.frame() %>% dplyr::mutate('PROPERTY TYPE - GROUP' = base::toupper(paste0(category_desc, ' - ', intlGrp))) %>% dplyr::select(-c(category_desc, intlGrp))
q40_multiYear_noINTL_table <- q40_multiYear_noINTL[,c(17,1:16)]

write.csv(q40_multiYear_noINTL_table, 'dataProd/Q40_exemptionStatus_USA_CANADA_all.csv', row.names = F)
